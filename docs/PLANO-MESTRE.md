...
| **Pentest autenticado (Agente 2)** | 🟡 Fase 3.1 (2026-08-12) | web-auth funcional: harness de sessão + cérebro do Agente 2 (`core/agent-2-authenticated.md`: autentica → `post-auth-mapper` → IDOR/BOLA/BFLA → relatório autenticado) + guidance web-auth próprio + receita de login. Falta o **handoff de um clique** (3.2) e o **progresso do Playwright** (3.3). |
...
| **3** | **Agente 2 — pentest autenticado + harness de sessão** 🟡 | **Fase 3.1 FEITA (2026-08-12):** harness de conexão (`session-harness.js` + `/harness` — prova login/sessão antes de gastar token), crawler autenticado (cérebro do Agente 2 em `core/agent-2-authenticated.md`), guidance web-auth próprio (fim do bug de receber guidance de Azure) e receita de login opcional. Roda pelo fluxo **web-auth (engagement separado)**. **Falta:** 3.2 **gatilho de handoff de um clique** (promover black-box→autenticado em runtime, semear shadow_graph) + 3.3 **progresso do Playwright** na timeline. Ver §5.2 e ROADMAP-CONSOLIDADO §8. | nada |
