...
const REGISTRY = [
  ...
  },
  {
    ...
    systemPrompt: [
      ...
      'WEB_AUTH_JWT e/ou WEB_AUTH_TOTP (2FA). Quando presentes, WEB_AUTH_LOGIN_URL,',
      'WEB_AUTH_USER_FIELD, WEB_AUTH_PASS_FIELD e WEB_AUTH_SUCCESS são a "receita de login"',
      'já validada pelo operador (harness de sessão) — use-a para autenticar sem',
      'redescobrir o formulário. Autentique na aplicação e cubra a superfície PÓS-LOGIN',
      '(não só a externa/black-box), seguindo core/agent-2-authenticated.md. Trate as',
      'credenciais como sensíveis: nunca as inclua em findings, logs ou relatórios.',
      'Respeite o escopo (domínios autorizados) e os checkpoints por fase.',
    ].join('\n'),
    credentialSpec: {
      fields: [
        ...
        // Receita de login (opcional) — o que o operador provou no harness de sessão.
        // Deixa o Agente 2 autenticar de forma determinística, sem adivinhar o formulário.
        { name: 'loginUrl',      label: 'URL de login (opcional)',                    secret: false, optional: true },
        { name: 'userField',     label: 'Campo do usuário — name= (opcional)',        secret: false, optional: true },
        { name: 'passField',     label: 'Campo da senha — name= (opcional)',          secret: false, optional: true },
        { name: 'successMarker', label: 'Marcador de sucesso pós-login (opcional)',   secret: false, optional: true },
      ],
      env: {
        ...
        jwt:           'WEB_AUTH_JWT',
        totp:          'WEB_AUTH_TOTP',
        loginUrl:      'WEB_AUTH_LOGIN_URL',
        userField:     'WEB_AUTH_USER_FIELD',
        passField:     'WEB_AUTH_PASS_FIELD',
        successMarker: 'WEB_AUTH_SUCCESS',
      },
    },
  },
  {
